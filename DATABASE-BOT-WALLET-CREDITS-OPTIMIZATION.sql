-- ============================================
-- FarBump - Bot Wallet Credits Optimization
-- ============================================
...
-- Step 1: Merge duplicate records (sum weth_balance_wei for each bot_wallet_address)
...
SELECT 
  ...
  MAX(created_at) AS updated_at -- Keep latest as updated_at
...
-- Step 3: Insert merged records back
...
-- Step 5: Drop column distributed_amount_wei (if exists)
ALTER TABLE bot_wallet_credits
DROP COLUMN IF EXISTS distributed_amount_wei;

-- Step 6: Add unique constraint on (user_address, bot_wallet_address)
...
-- Step 8: Update function to use weth_balance_wei only
...
-- ============================================
-- Changes:
--   1. Merged duplicate records (summed weth_balance_wei)
--   2. Removed distributed_amount_wei column
--   3. Added unique constraint on (user_address, bot_wallet_address)
--   4. Updated indexes for better performance
--   5. Updated get_total_bot_wallet_credits function to use weth_balance_wei only
-- 
-- Result:
